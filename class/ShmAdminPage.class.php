...
class ShmAdminPage
{
    ...
    function show_admin_footer_on_default_pages($old_footer_html='') {
        ...
        //if( !empty($_GET['post_type']) && in_array($_GET['post_type'], array('shm_map', 'shm_point', 'shm_request')) ) {
    ...
    function get_admin_footer($footer_class='', $old_footer_html='') {
        ob_start();
        ?>
        <span class="shmapper-admin-footer <?php echo $footer_class;?>">
            <a href="https://t.me/shmapper" target="_blank"><?php _e('Shmapper developers chat', 'leyka');?></a>
        </span>
        <?php
